...
export const updateProductStatus = (productId, active) => {
  const url = `/api/admin/products/${productId}/update-status?active=${active}`;
  return Instance.put(url);
};
export const getAllProductsCloseByBrand = (brand, page, size, active) => {
  const url = `/api/site/product/by-brand?brand=${brand}&page=${page}&size=${size}&active=${active}`;
  return Instance.get(url);
};
